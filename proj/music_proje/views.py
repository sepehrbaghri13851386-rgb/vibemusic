...
def artists(request):
    ...
    # روش صحیح برای گرفتن نام‌های یکتا
    # استفاده از distinct() روی فیلد name
    names = tracks.values_list('name', flat=True).distinct()
    
    # اگر نام‌ها با املای مختلف ذخیره شده‌اند، می‌توانید از lower() استفاده کنید
    # اما بهتر است در دیتابیس نام‌ها را یکسان کنید
    ...
    artist_list = []
    seen_names = set()  # برای اطمینان بیشتر از عدم تکراری بودن
    
    for name in names:
        # اگر نام قبلاً دیده شده، رد کن
